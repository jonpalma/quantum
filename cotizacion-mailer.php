<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $name = strip_tags(trim($_POST["name-2"]));
		$name = str_replace(array("\r","\n"),array(" "," "),$name);
        $tel = trim($_POST["phone-2"]);
        $email = filter_var(trim($_POST["email-2"]), FILTER_SANITIZE_EMAIL);
        ...
        $recipient = "contacto@quantumdesarrollos.com.mx";

         // Set the email subject.
        $subject = "Solicitud de Cotización para $name";
		$newsubject='=?UTF-8?B?'.base64_encode($subject).'?=';// Set the email subject.

        // Build the email content.
        $email_content = "Mensaje enviado desde la forma de cotización de Quantum\n";
